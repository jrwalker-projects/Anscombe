---
title: "Summary Statistics & Data Visualization"
author: "John Walker (derivative work)"
...
## Data visualization example using the Anscombe dataset
...
[Francis Anscombe](https://en.wikipedia.org/wiki/Frank_Anscombe), an English statistician, designed a dataset to illustrate how data visualization helps people see how different 
data can be when summary statistics match. 

For R programming the Anscombe data is included in the base `datasets` package. Looking at the data the differences
in the 4 sets is not easy to detect.
```{r libs, warning=FALSE, message=FALSE, echo=FALSE}
require(ggplot2) ; require(dplyr) ; require(reshape2); require(xtable); require(kable); require(plotly)
```
```{r data, echo=FALSE}
anscombe
```
